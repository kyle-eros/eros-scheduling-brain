#!/usr/bin/env python3
"""
Clean and prepare PPV caption data for BigQuery loading
"""
import pandas as pd
import sys
...
def clean_ppv_data():
    # Read the raw PPV CSV file
    df = pd.read_csv('/Users/kylemerriman/Desktop/eros-data-pipe/data/EROS CAPTION LOGGER  - MASS MESSAGE PPV.csv')
    ...
    cleaned_df['price_last_sent'] = df['PriceLastSent'].str.replace('$', '').astype(float)
    ...
    # Save cleaned data
    cleaned_df.to_csv('/Users/kylemerriman/Desktop/eros-data-pipe/data/cleaned_ppv_captions.csv', index=False)
    print(f"Cleaned {len(cleaned_df)} PPV captions")
