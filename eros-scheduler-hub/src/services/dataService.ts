...
const intParam = (name: string, value: number): GoogleAppsScript.BigQuery.Schema.QueryParameter => ({
  name,
...
export interface TodayRow {
  recommended_send_ts: string;
  local_time: string;
  username_std: string;
  page_handle: string;
  page_type: string;
  full_tier_assignment: string;
  message_type: string;
  ...
  fatigue_safety_score: number;
  opportunity_quality: string;
  recommendation_reason: string;
  time_energy_required: string;
...
export interface AuthenticityAlert {
  ...
  exact_time_repetitions_30d: number;
  avg_spacing_minutes_30d: number;
}
...
export const DataService = {
  getIdentity(): SchedulerIdentity {
    ...
    const sql = `
      ...
      LIMIT 1
    `;
    const rows = bq.query<SchedulerIdentity>(sql, [stringParam('email', email)]);
    if (!rows.length) {
  ...
  getHomeTiles(email: string): HomeTile[] {
    const sql = `
      WITH my AS (
        SELECT username_std
        FROM \`${source}.${tables.tierAssignments}\`
  ...
  getTodayPlan(email: string): TodayRow[] {
    const sql = `
      WITH my AS (
        SELECT username_std
        FROM \`${source}.${tables.tierAssignments}\`
    ...
    `;
    return bq.query<TodayRow>(sql, [stringParam('email', email), stringParam('tz', timezone)]);
  },

  getWeekPlan(email: string): WeekRow[] {
    const sql = `
      WITH my AS (
        SELECT username_std
        FROM \`${source}.${tables.tierAssignments}\`
    ...
    `;
    return bq.query<WeekRow>(sql, [stringParam('email', email), stringParam('tz', timezone)]);
  },

  getAuthenticityAlerts(email: string): AuthenticityAlert[] {
    const sql = `
      WITH my AS (
        SELECT username_std
        FROM \`${source}.${tables.tierAssignments}\`
      ...
      LIMIT 25
    `;
    return bq.query<AuthenticityAlert>(sql, [stringParam('email', email), stringParam('tz', timezone)]);
  },

  getPerformanceMetrics(email: string): PerformanceMetric[] {
    const sql = `
      SELECT
        COUNT(*) AS actions_7d,
  ...
  getCaptionSuggestions(creator: string, hod: number, slotDate: string): CaptionSuggestion[] {
    const params = [stringParam('creator', creator), intParam('hod', hod), stringParam('slotDate', slotDate)];
    const first = bq.query<CaptionSuggestion>(
      `SELECT
    ...
    const second = bq.query<CaptionSuggestion>(
      `SELECT
         CAST(caption_id AS STRING) AS caption_id,
         caption_text,
    ...
    const third = bq.query<CaptionSuggestion>(
      `SELECT
         NULL AS caption_id,
  ...
  getRecentActions(email: string): Array<Array<unknown>> {
    const sql = `
      SELECT
        action_ts,
        username_std,
